...
- **Key logic:** **CHỈ 1 lớp chạy nền tại một thời điểm** — `applyBackgroundTriggers` (gọi ở `main._bootstrap` + khi đổi cài đặt nền): nếu FG **bật** (mặc định) → chỉ `startWeatherForegroundService`, **hủy** `cancelWeatherAlarm` + `BackgroundScheduler.cancel` (tránh 3 lớp cùng đá máy dậy mỗi chu kỳ gây **nóng máy**); nếu FG **tắt** → `stopWeatherForegroundService` + đăng ký alarm exact + WorkManager. **Foreground service** — thông báo thường trực (channel `weather_foreground` importance LOW) live `foregroundStatusText` (emoji + °C + tình hình + UV + **giờ cập nhật HH:mm**), chu kỳ = `intervalMinutes` đọc từ prefs (5/10/15/30'), **`allowWifiLock=false`** + `allowWakeLock` (nguồn wakelock duy nhất). **Alarm exact** (`kWeatherAlarmId=2001`, `oneShotAt exact+allowWhileIdle`) — re-arm theo chu kỳ **chỉ khi FG tắt** (không tự nhân bản khi FG bật). **WorkManager periodic** clamp **≥15'** (tối thiểu Android). Cả 3 đổ về `runWeatherCheck` (isolate riêng, tự dựng DI). **GUARD QUOTA bám theo chu kỳ:** chỉ gọi API khi cache cũ ≥ `intervalMinutes − 1'` (đọc `BackgroundPrefsStore`; trước đây cứng 12' để khử 3 lớp — nay 1 lớp nên bám chu kỳ để 5' thật sự làm mới mỗi 5'); không vượt hạn mức 1000 call/ngày (3 call/refresh — chu kỳ ngắn tốn hạn mức nhanh). **Guard dữ liệu cũ:** bỏ sinh cảnh báo nếu `data.age > alertMaxDataAgeMinutes=45'`. Sinh **3 nhóm** thông báo: (1) thời điểm mưa (RainStatus), (2) tình hình (WeatherCondition), (3) nhiệt/ẩm (EnvChange). Chống spam: chỉ phát khi PHA mưa / NHÓM thời tiết đổi so với `AlertStateStore` (phase + category + **changeAt** + envNotified) — **ngoại lệ:** pha giữ nguyên nhưng `changeAt` lệch ≥ `rainTimeShiftRenotifyMinutes=15'` → phát **"Cập nhật: …"** (cùng ID → thay thế). Nội dung mưa kèm **giờ HH:MM từ `rain.changeAt`** + **% khả năng mưa** + **giờ tạnh/thời lượng** (`_durationSuffix` ← `rain.rainEndsAt`/`durationMinutes`). `RainAlertBanner` hiển thị cùng nội dung. `BigTextStyleInformation` chống cắt chữ.
...
- **Key logic:** kênh thông báo **độc lập** với cảnh báo sự kiện — tóm tắt nhiệt độ + cảm giác như + hi/lo 24h + tình hình `WeatherCondition` + lời khuyên + **outlook mưa CẢ NGÀY theo buổi** (`BuildRainOutlook`) + gợi ý mưa tức thời từ `AnalyzeRain` (giờ HH:MM từ `changeAt` + % + **giờ tạnh**) + **UV kèm lời khuyên theo mức** (`UvAdvice`) + "Cập nhật lúc HH:MM". **NHIỀU MỐC GIỜ TÙY Ý:** `DigestPrefs.times` (List phút-trong-ngày, sort+dedupe qua `normalizeTimes`, kẹp ≤ `digestMaxSlots`); người dùng thêm/xóa trong `DigestSettingsCard` ở màn Thời tiết (bê từ Settings sang); **migrate** tự động từ key cũ `digest_morning_min`/`digest_evening_min` → `digest_times` khi đọc lần đầu. Mỗi mốc index `i` → alarm ID `digestBase + i`; `digestAlarmCallback` giải mã `index = id − digestBase`, hiển thị rồi **re-arm cùng id** với `times[index]` (không re-arm nếu index đã bị xóa/`!enabled`). **Cơ chế = `AndroidAlarmManager.oneShotAt`** (`wakeup + allowWhileIdle + rescheduleOnReboot`) — KHÔNG dùng `periodic` (setRepeating INEXACT bị Doze hoãn → bản tin sáng không nổ). **FIX "đến giờ không nổ":** trước khi đặt, `scheduleDigestSlot` kiểm tra `canScheduleExactAlarms` (`ph.Permission.scheduleExactAlarm`); **thiếu quyền → `exact:false`** (inexact, vẫn nổ gần đúng giờ thay vì ném `SecurityException` bị nuốt âm thầm) + `debugPrint` cảnh báo; `DigestSettingsCard` hiện dòng cảnh báo + nút xin quyền (`requestExactAlarmPermission`). `scheduleDigests` **idempotent** (hủy `digestMorning/Evening` cũ + toàn dải `digestBase..+maxSlots` trước khi đặt) → gọi lại ở **main `_bootstrap`**, **`NotificationSettingsController`**, **`runWeatherCheck`** (mỗi chu kỳ) **tự chữa** chuỗi one-shot bị đứt. Tại mốc giờ, callback (isolate riêng) **FETCH dữ liệu tươi rồi mới hiển thị**. Công tắc `enabled` (tắt → hủy toàn dải). **AndroidManifest** khai báo `USE_EXACT_ALARM`/`SCHEDULE_EXACT_ALARM`.
