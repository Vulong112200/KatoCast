...
- `mobile/lib/core/diagnostics/app_log.dart` (**AppLog** — nhật ký JSONL bền, dùng ở mọi isolate, KHÔNG bao giờ ném lỗi; rotation + prune) + `log_entry.dart` / `log_tags.dart` / `log_health.dart` (**LogHealth** suy tình trạng + `longestGap24h`)
...
- `mobile/lib/core/background/cycle_lock.dart` (**`CycleLock`** — lock BẰNG FILE chống chạy chồng giữa các isolate; sửa cả `database is locked` lẫn thông báo trùng)
...
- `mobile/lib/core/background/foreground_service.dart` (foreground service `flutter_foreground_task` — chu kỳ từ prefs, `allowWifiLock=false`, toàn bộ phần dùng DB trong `CycleLock`, ngoài khung giờ vẫn cập nhật thông báo "đang nghỉ"; `startWeatherForegroundService({allowRestart})`)
- `mobile/lib/core/background/weather_alarm.dart` (alarm exact BACKSTOP luôn tự re-arm; **`kNightHopInterval=2h`** chặng đêm thay cho một alarm treo cả đêm; `weatherAlarmChainStatus()` cho watchdog WorkManager; hồi sinh FG chỉ khi đã chết)
...
- `mobile/lib/core/background/background_worker.dart` (WorkManager **LUÔN bật** — lịch do JobScheduler của HĐH giữ nên sống sót khi chuỗi one-shot đứt; clamp ≥15', dựng lại alarm khi `overdue`)
